...
class Pendulo :
    ...
    def plot(self):
        ...
        ax[2].plot(self.tau, self.energy(),  marker='o', linestyle='None',
                   color='red', label="Energia")
    ...
    def energy(self):
        return (0.5*self.masa*self.longitud**2 *
                (self.omega()**2 +
                    (self.gravedad/self.longitud)*self.theta()**2))
    ...
    def integrate(self, num_steps, t_i, t_f, method):
        ...
        for j in range(num_steps-1):
            self.trajectory[j+1] =\
                method(self.trajectory[j], self.tau[j], self.dt, self.dynamics)
